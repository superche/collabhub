import { DraftCommandBus } from '../application/draft-command-bus.js';
...
export function createDraftApplication() {
    ...
    let diagnosticsUnsubscribe = () => undefined;
    ...
    const attach = (next) => {
        ...
        if (transport instanceof CollabHubDraftTransport)
            diagnosticsUnsubscribe = transport.subscribeDiagnostics(() => { for (const listener of diagnosticsListeners)
                listener(); });
    };
    const make = (collab) => collab
