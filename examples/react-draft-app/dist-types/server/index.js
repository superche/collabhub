...
import { PROTOCOL_VERSION } from '@collabhub/protocol';
...
const socketsByDocument = new Map();
const app = express();
app.use(cors());
...
webSockets.on('connection', (socket) => {
    ...
    socket.on('message', async (raw) => {
        ...
        if (message.kind === 'hello') {
            ...
            const roomSockets = socketsByDocument.get(documentId) ?? new Set();
            roomSockets.add(socket);
            socketsByDocument.set(documentId, roomSockets);
            const session = await core.session(message.tenantId, message.documentId);
            unsubscribe = session.subscribe((event) => { if (socket.readyState === socket.OPEN)
        ...
        for (const peer of socketsByDocument.get(documentId) ?? [])
            if (peer !== socket && peer.readyState === peer.OPEN)
                peer.send(JSON.stringify(presence));
    });
    socket.on('close', () => {
        unsubscribe();
        if (!documentId)
        ...
        if (next === 0)
            activeCounts.delete(documentId);
        else
            activeCounts.set(documentId, next);
