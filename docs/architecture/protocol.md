...
Wire 类型在 `packages/protocol/src/index.ts`。WebSocket 只承载 JSON 编码，不定义协议语义。
...
resolve/invariant 必须确定、无 I/O、无副作用。v0.1 是 trusted in-process extension，不运行不可信插件。hook 抛错 fail closed，结果为 `strategyFailure`。
