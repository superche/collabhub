...
test('React Flow converges incremental graph operations and offline replay', async ({ browser }) => {
  ...
  await expect(bob.getByTestId('react-flow-version')).toHaveText('3')
  ...
  await expect(alice.locator('.react-flow__node')).toHaveCount(6)
  await expect(bob.locator('.react-flow__node')).toHaveCount(6)
  await expect(alice.getByTestId('react-flow-version')).toHaveText('5')

  await alice.locator('[data-id="build"]').click()
  await alice.getByTestId('delete-selection').click()
  await expect(alice.getByText('5 nodes · 0 edges', { exact: true })).toBeVisible()
  await expect(bob.getByText('5 nodes · 0 edges', { exact: true })).toBeVisible()
  await expect(bob.getByTestId('react-flow-version')).toHaveText('6')
