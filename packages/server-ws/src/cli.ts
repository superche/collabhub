...
const handle = await startJsonCollaborationServer({
  ...
  initialState: (documentId) => ({ ...initialState, id: documentId }),
...
console.log(JSON.stringify({ event: 'collabhub_standalone_ready', port: handle.port, path: '/collab' }))
for (const signal of ['SIGINT', 'SIGTERM'] as const) {
  process.on(signal, () => { void handle.close().finally(() => process.exit(0)) })
}

function parseInitialState(value: string | undefined): Record<string, unknown> {
  if (!value) return {}
