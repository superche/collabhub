...
try {
  ...
  await humanClick(alice, alice.getByTestId('add-node'))
  await waitForNodeCount(alice, 5)
  ...
  await Promise.all([waitForNodeCount(alice, 6), waitForNodeCount(bob, 6)])
  await converge(alice, bob)
  record('reconnect_converged', await diagnostics(alice, bob))
  await pause(1_300)

  await activate(alice, bob, 'Delete Build + both linked edges atomically')
  await humanClick(alice, alice.locator('[data-id="build"]'))
  await humanClick(alice, alice.getByTestId('delete-selection'))
  await Promise.all([waitForSummary(alice, '5 nodes · 0 edges'), waitForSummary(bob, '5 nodes · 0 edges')])
  await converge(alice, bob)
