...
try {
  ...
  await landing.goto(`http://127.0.0.1:${port}/`)
  ...
  expect(await landing.getByRole('link', { name: 'Try two-client demo' }).evaluate((element) => getComputedStyle(element).backgroundColor)).toBe('rgb(23, 92, 211)')
  expect(await landing.locator('.keep-grid .accent-card').evaluate((element) => getComputedStyle(element).boxShadow)).toBe('none')
  expect(await landing.locator('.open-source-section').evaluate((element) => getComputedStyle(element).boxShadow)).toBe('none')
  await expect(landing.locator('link[rel="icon"]')).toHaveAttribute('href', '/collabhub-mark.svg')
  const favicon = await fetch(`http://127.0.0.1:${port}/collabhub-mark.svg`)
  expect(favicon.ok).toBe(true)
  expect(await favicon.text()).toContain('#175cd3')
  await landing.getByRole('button', { name: '中文' }).click()
  await expect(landing.getByRole('heading', { name: '无需重写 React 应用，也能多人协作。' })).toBeVisible()
  await expect(landing.getByRole('link', { name: '查看源码' })).toBeVisible()
  expect(await landing.locator('html').getAttribute('lang')).toBe('zh-CN')
  expect(new URL(landing.url()).searchParams.get('lang')).toBe('zh')
  ...
  await reopened.goto(`http://127.0.0.1:${port}/room?document=${documentId}&client=reopened`)
